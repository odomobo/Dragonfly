...
namespace SharpHeart.Engine.MoveGen
{
    ...
    internal static class PawnQuietMoveTable
    {
        ...
        private static readonly MagicMoveTable[] QuietMovesMagicTables;
        
        static PawnQuietMoveTable()
        {
            QuietMovesMagicTables = new MagicMoveTable[2];
            ...
            foreach (var color in new[] {Color.White, Color.Black})
            {
                ...
                try
                {
                    QuietMovesMagicTables[(int) color] = new MagicMoveTable(
                        masks,
                        Magics[(int) color],
                        occupancies,
                        moves,
                ...
                catch (Exception e)
                {
                    ...
                    QuietMovesMagicTables[(int)color] = new MagicMoveTable(masks, dynamicMagics, occupancies, moves, MaxMaskSize);
                }
            }
        }
        ...
        {
            return QuietMovesMagicTables[(int) color].GetMoves(ix, occupancy);
        }
        ...
        private static (ulong mask, ulong[] occupancies, ulong[] moves) GetSingleMaskOccupanciesMoves(int srcFile, int srcRank, Color color)
        {
            ...
            {
                mask |= Board.ValueFromFileRank(srcFile, srcRank + (direction * 2));
            }
            ...
            for (int permutation = 0; permutation < maskPermutations; permutation++)
            {
                ...
                dstRank = srcRank + direction;
                if (Board.FileRankOnBoard(srcFile, dstRank))
                {
                    if ((occupancy & Board.ValueFromFileRank(srcFile, dstRank)) > 0)
                    {
                        blocked = true;
                    }
                    else
                ...
                if (!blocked && IsStartingRank(srcRank, color))
                {
                    dstRank = srcRank + (direction * 2);

                    if ((occupancy & Board.ValueFromFileRank(srcFile, dstRank)) > 0)
                    {
                        blocked = true; // not really necessary
                    }
                    else
                    {
                        singularMoves |= Board.ValueFromFileRank(srcFile, dstRank);
